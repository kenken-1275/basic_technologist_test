...
CPUはメモリから命令やデータを取り出し実行、結果をまたメモリに格納する、ということを繰り返す。<br>
...
### CPUの構成要素
CPUはさまざまな機構で構成されている。中でも重要なのは、レジスタ・ALU・デコーダ。<br>
...
### 周辺装置とハードウェア
- 入出力インターフェース<br>
周辺機器を接続するための規格を入出力インターフェースという。各規格では、コネクタの形状や周辺装置とのデータ転送手順などを定義している。<br>
近年は、1本の線上に1ビットずつ順次に送り込むシリアル伝送という方式で高速伝送を実現している。
<br />

- USB<br>
最大で127台までツリー状に接続できる。USB2.0では最大480Mbps、USB3.0では最大5Gbpsでの伝送が可能。ちなみにUSB3.0は「スーパースピードと呼ばれる5Gbpsのデータ転送モードを持つシリアルインターフェース」と表現されることがある。<br>
<br />

- シリアルATA(SATA)<br>
主に内臓用のハードディスク装置やDVDドライブの接続に使われる。シリアル伝送により、1.5Gbps 〜 6Gbpsでの伝送を行う。<br>
<br />

- Bluetooth<br>
無線機器の接続に使う。2.4GHz帯の電波を使う。省電力化のための仕様として、BLE(Bluetooth Low Energy)がある。<br>
<br />

- 組み込みシステム<br>
機器に組み込まれて、コンピュータが裏方として働いているシステムを組み込みシステムという。<br>
洗濯機や自動車など、センサからの信号に対して、瞬時に反応し処理を行わなくてはならない場面が多い。このような場面では、RTOS（リアルタイムOS）という特別なOSが利用されている。<br>
<br />

- 組み込みシステム関連用語<br>
・IoT(Internet of Things)：組み込みシステムをインターネットに接続し、遠隔地からデータ収集・分析、機器のコントロールを行うシステム。<br>
・SoC(System on a Chip)：CPUと周辺回路を半導体の1チップ上に集積したもの。これにより部品数を減らすことができ、機器の小型化ができる。（スマホなど）<br>
・Sip(System in Package)：半導体の1チップで集積できないような大規模システムの場合に、複数のチップを1つのICパッケージに格納する技術。SoCよりも大規模なシステムを実現できる。<br>
<br />
