...
### ソフトウェア開発に用いるツール
- プログラミング言語：プログラムを記述するための言語。人間にとって分かりやすい表記という点で、機械語との比較で、高水準言語という。逆に機械語は低水準言語という。
- 言語プロセッサ：アセンブラ・コンパイラ・インタプリタがある。
- アセンブラ：アセンブリ言語で書かれたプログラムを機械語に変換するプログラム。
- コンパイラ：CやJavaなどの言語で書かれたプログラムを機械語に変換するプログラム。プログラム全体をまとめて機械語に変換してから実行するため、インタプリタより実行速度が速い。
- インタプリタ：プログラムを１行ずつ解釈して実行する。RubyやPythonなどの言語で書かれたプログラムを実行する。１行ずつ実行するため、コンパイラより遅いが、途中までしか出来上がっていなくてもそこまでは実行できるため、変数の内容チェックなどしやすい。
<br />

#### コンパイルの流れ
字句解析→構文解析→意味解析→最適化<br>
- 字句解析：プログラムを単語に分ける作業。
- 構文解析：言語の文法に従って単語の並びを分析する作業。
- 意味解析：変数や演算子の意味をとらえて分析する作業。
- 最適化：処理を単純化したり、より高速な命令を使うようにしたりして、プログラムの実行時間を短縮するための作業。
